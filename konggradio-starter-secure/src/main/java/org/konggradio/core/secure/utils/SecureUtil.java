 ...
 *
 */
package org.konggradio.core.secure.utils;
...
import org.konggradio.core.launch.constant.TokenConstant;
import org.konggradio.core.secure.KongGradioUser;
import org.konggradio.core.secure.TokenInfo;
import org.konggradio.core.secure.constant.SecureConstant;
import org.konggradio.core.secure.exception.SecureException;
import org.konggradio.core.secure.provider.IClientDetails;
...
public class SecureUtil {
	private static final String KONG_CLOUD_USER_REQUEST_ATTR = "_KONG_CLOUD_USER_REQUEST_ATTR_";
	...
	public static KongGradioUser getUser() {
		HttpServletRequest request = WebUtil.getRequest();
		if (request == null) {
			return null;
		}
		// 优先从 request 中获取
		Object attribute = request.getAttribute(KONG_CLOUD_USER_REQUEST_ATTR);
		if (attribute == null) {
			attribute = getUser(request);
			if (attribute != null) {
				// 设置到 request 中
				request.setAttribute(KONG_CLOUD_USER_REQUEST_ATTR, attribute);
			}
		}
		return (KongGradioUser) attribute;
	}


	public static KongGradioUser getUser(HttpServletRequest request) {
		...
		KongGradioUser kongGradioUser = new KongGradioUser();
		kongGradioUser.setClientId(clientId);
		kongGradioUser.setUserId(userId);
		kongGradioUser.setTenantId(tenantId);
		kongGradioUser.setAccount(account);
		kongGradioUser.setRoleId(roleId);
		kongGradioUser.setRoleName(roleName);
		kongGradioUser.setUserName(userName);
		return kongGradioUser;
	}
	...
	public static Long getUserId(HttpServletRequest request) {
		KongGradioUser user = getUser(request);
		return (null == user) ? -1 : user.getUserId();
	}
