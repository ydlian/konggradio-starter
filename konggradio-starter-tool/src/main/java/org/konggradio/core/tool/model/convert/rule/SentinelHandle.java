...
public class SentinelHandle {
     ...
     * Flow rule enable.
     */
    private Integer flowRuleEnable = PluginConstants.SENTINEL_ENABLE_FLOW_RULE;

    /**
     * Flow rule grade.
     */
    private Integer flowRuleGrade = PluginConstants.SENTINEL_QPS_FLOW_GRADE;
     ...
     */
    private Integer flowRuleControlBehavior = PluginConstants.SENTINEL_FLOW_REJECT;

    /**
     * Degrade rule control behavior.
     */
    private Integer degradeRuleEnable = PluginConstants.SENTINEL_ENABLE_DEGRADE_RULE;

    /**
     * Degrade rule grade.
     */
    private Integer degradeRuleGrade = PluginConstants.SENTINEL_RESPONSE_RULE_GRADE;
     ...
     */
    private Integer degradeRuleMinRequestAmount = PluginConstants.SENTINEL_MIN_REQUEST_AMOUNT;

    /**
     * Degrade rule slow ratio threshold.
     */
    private Double degradeRuleSlowRatioThreshold = PluginConstants.SENTINEL_SLOW_RATIO_THRESHOLD;

    /**
     * Degrade rule stat intervalMs.
     */
    private int degradeRuleStatIntervals = PluginConstants.SENTINEL_STAT_INTERVALS;
    ...
    public void checkData(final SentinelHandle sentinelHandle) {
        sentinelHandle.setFlowRuleEnable((sentinelHandle.getFlowRuleEnable() == 1 || sentinelHandle.getFlowRuleEnable() == 0)
                ? sentinelHandle.getFlowRuleEnable() : PluginConstants.SENTINEL_ENABLE_FLOW_RULE);
        sentinelHandle.setDegradeRuleEnable((sentinelHandle.getDegradeRuleEnable() == 1 || sentinelHandle.getDegradeRuleEnable() == 0)
                ? sentinelHandle.getDegradeRuleEnable() : PluginConstants.SENTINEL_ENABLE_DEGRADE_RULE);
    }
}
