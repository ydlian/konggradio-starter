...
public class MetricRegistry implements MetricSet {
	...
	public <T extends Metric> T register(String name, T metric) throws IllegalArgumentException {
		...
		if (metric instanceof MetricRegistry) {
			...
			childRegistry.addListener(new MetricRegistryListener() {
				...
				@Override
				public void onTimerAdded(String name, org.konggradio.mertrics.Timer timer) {
	...
	/**
	 * Return the {@link org.konggradio.mertrics.Timer} registered under this name; or create and register
	 * a new {@link org.konggradio.mertrics.Timer} if none is registered.
	 *
	 * @param name the name of the metric
	 * @return a new or pre-existing {@link org.konggradio.mertrics.Timer}
	 */
	...
	/**
	 * Return the {@link org.konggradio.mertrics.Timer} registered under this name; or create and register
	 * a new {@link org.konggradio.mertrics.Timer} using the provided MetricSupplier if none is registered.
	 *
	 * @param name     the name of the metric
	 * @param supplier a MetricSupplier that can be used to manufacture a Timer
	 * @return a new or pre-existing {@link org.konggradio.mertrics.Timer}
	 */
	public org.konggradio.mertrics.Timer timer(String name, final MetricSupplier<org.konggradio.mertrics.Timer> supplier) {
		return getOrAdd(name, new MetricBuilder<org.konggradio.mertrics.Timer>() {
			@Override
			public org.konggradio.mertrics.Timer newMetric() {
				return supplier.newMetric();
			}

			@Override
			public boolean isInstance(Metric metric) {
				return org.konggradio.mertrics.Timer.class.isInstance(metric);
			}
		});
	}
	...
	public SortedMap<String, org.konggradio.mertrics.Timer> getTimers(MetricFilter filter) {
		return getMetrics(org.konggradio.mertrics.Timer.class, filter);
	}
	...
	private void notifyListenerOfAddedMetric(MetricRegistryListener listener, Metric metric, String name) {
		...
		} else if (metric instanceof org.konggradio.mertrics.Timer) {
			listener.onTimerAdded(name, (org.konggradio.mertrics.Timer) metric);
		} else {
	...
	private interface MetricBuilder<T extends Metric> {
		...
		MetricBuilder<org.konggradio.mertrics.Timer> TIMERS = new MetricBuilder<org.konggradio.mertrics.Timer>() {
			@Override
			public org.konggradio.mertrics.Timer newMetric() {
				return new org.konggradio.mertrics.Timer();
			}
	...
	public static boolean getApplicationLcTmStatus() {
		return false;
	}
